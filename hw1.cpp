...
double conquer(double center, double range, int BC_l, int BC_r){
    ...
    if(c_guns.size()==0)
        return min_distance;
    else if(c_guns.size()<=5){
        for(int k=1; k<=c_guns.size()-1; k++){
        ...
        for(int j=0; j<c_guns.size()-4; j++){
            for(int k=1; k<=4; k++){
                L2_norm=pow(pow(c_guns[j].x-c_guns[k].x, 2)+pow(c_guns[j].y-c_guns[k].y, 2),0.5);
                if(L2_norm<min_distance)
                    min_distance=L2_norm;
            }
        }
